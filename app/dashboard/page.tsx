...
interface Claim {
  ...
  branch: string;
  amount: number;
  status: "pending" | "approved" | "paid" | "rejected" | "superseded";
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex rounded-full overflow-hidden h-3 gap-0.5">
        <div
          ...
          style={{ width: `${(bucket0 / total) * 100}%` }}
        />
        <div
          className="bg-amber-400 transition-all"
          style={{ width: `${(bucket31 / total) * 100}%` }}
        />
        <div
          className="bg-red-500 transition-all"
          style={{ width: `${(bucket60 / total) * 100}%` }}
        />
      </div>
      <div className="flex gap-4 mt-3 flex-wrap">
        {[
          { label: "0–30 days", color: "bg-emerald-400", amount: bucket0 },
          { label: "31–60 days", color: "bg-amber-400", amount: bucket31 },
          { label: "60+ days", color: "bg-red-500", amount: bucket60 },
        ].map((b) => (
...
function MedicalAidTable({
  ...
  data: Record<string, { pending: number; approved: number; total: number }>;
}) {
      ...
      {sorted.length === 0 ? (
        ...
        <div className="space-y-3">
          {sorted.map(([aid, vals]) => {
            ...
            const pct = (vals.total / maxVal) * 100;
            return (
              <div key={aid}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-gray-800">{aid}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-amber-600">
                      {formatUSD(vals.pending)} pending
                    </span>
                    <span className="text-xs text-blue-600">
                      {formatUSD(vals.approved)} approved
                    </span>
                    <span className="text-sm font-bold text-gray-900">
                      {formatUSD(vals.total)}
                    </span>
                  </div>
                </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-red-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/claims?status=rejected">
...
export default function DashboardPage() {
  ...
  // Outstanding = pending + approved ONLY
  ...
  const bucket31 = outstanding.filter((c) => {
    const d = getDaysOutstanding(c.submissionDate);
    return d > 30 && d <= 60;
  });
  const bucket60 = outstanding.filter(
    (c) => getDaysOutstanding(c.submissionDate) > 60
  );

  // Per medical aid (outstanding only, split by status)
  const byAid: Record<
    string,
    { pending: number; approved: number; total: number }
  > = {};
  outstanding.forEach((c) => {
    if (!byAid[c.medicalAid])
      byAid[c.medicalAid] = { pending: 0, approved: 0, total: 0 };
    if (c.status === "pending")  byAid[c.medicalAid].pending  += c.amount;
    if (c.status === "approved") byAid[c.medicalAid].approved += c.amount;
    byAid[c.medicalAid].total += c.amount;
  });

  const sum = (arr: Claim[]) => arr.reduce((s, c) => s + c.amount, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-gray-900 text-white rounded-2xl px-7 py-6 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">
              Total Outstanding
            </p>
            <p className="text-[11px] text-gray-500 mt-0.5">
              Pending + Approved — money owed to Link Optical right now
            </p>
            <p className="text-5xl font-bold mt-3 tabular-nums">
              {formatUSD(totalOutstanding)}
            </p>
            <p className="text-sm text-gray-400 mt-2">
              {outstanding.length} active claims across{" "}
              {Object.keys(byAid).length} medical aid
              {Object.keys(byAid).length !== 1 ? "s" : ""}
            </p>
          </div>
          <div className="hidden md:flex flex-col items-end gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">
                {pending.length} pending · {formatUSD(sum(pending))}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-blue-400" />
              <span className="text-xs text-gray-400">
                {approved.length} approved · {formatUSD(sum(approved))}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-red-400" />
              <span className="text-xs text-gray-400">
                {bucket60.length} overdue 60+ days
              </span>
            </div>
          </div>
        </div>
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">
            Active Claims — Money Being Tracked
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <StatCard
              label="Pending"
              sublabel="Submitted to 263, awaiting response"
              amount={sum(pending)}
              count={pending.length}
              countLabel="claims"
              accent="bg-amber-400"
              icon="🕐"
              hint="263 has not yet approved or rejected"
            />
            <StatCard
              label="Approved"
              sublabel="263 approved — waiting for payment"
              amount={sum(approved)}
              count={approved.length}
              countLabel="claims"
              accent="bg-blue-400"
              icon="✅"
              hint="Medical aid owes this money"
            />
            <StatCard
              label="Overdue 60+ Days"
              sublabel="Approved or pending over 60 days ago"
              amount={sum(bucket60)}
              count={bucket60.length}
              countLabel="claims"
              accent="bg-red-500"
              icon="⚠️"
              hint="Chase these today — call the medical aid"
            />
          </div>
        </div>

        {/* ── Row 2: Aging + Medical Aid Breakdown ── */}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <ResolvedCard
              ...
              sublabel={`${now.toLocaleString("default", {
                month: "long",
              })} ${now.getFullYear()} — confirmed received`}
              amount={sum(paidMonth)}
              count={paidMonth.length}
              countLabel="claims paid"
              icon="💰"
              accent="bg-emerald-500"
              hint="Revenue collected and confirmed"
            />
            <ResolvedCard
              label="Rejected"
              sublabel="Declined by 263 — needs resubmission"
              amount={sum(rejected)}
              count={rejected.length}
              countLabel="claims rejected"
              icon="❌"
              accent="bg-red-400"
              hint={
                rejected.length > 0
                  ? "Action needed — open claims to resubmit"
                  : "All clear"
              }
            />
          </div>
