...
// ─── Stat Card ────────────────────────────────────────────────────────────────
function StatCard({
  label, sublabel, amount, count, countLabel, accent, icon, hint,
}: {
...
function ResolvedCard({
  label, sublabel, amount, count, countLabel, icon, accent, hint,
}: {
  label: string; sublabel: string; amount: number; count: number;
...
// ─── Aging Bar ────────────────────────────────────────────────────────────────
function AgingBar({ bucket0, bucket31, bucket60 }: { bucket0: number; bucket31: number; bucket60: number }) {
  const total = bucket0 + bucket31 + bucket60 || 1;
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">Aging Breakdown</p>
  ...
  return (
    <div className="bg-white rounded-2xl border border-red-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs font-semibold uppercase tracking-widest text-red-400">⚠️ Rejected — Action Required</p>
          ...
          <div key={c._id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
            ...
            <div className="text-right">
              ...
              <Link href={`/claims/${c._id}/edit`}>
                <span className="text-xs text-blue-500 hover:underline cursor-pointer">Resubmit →</span>
              </Link>
            </div>
          </div>
        ))}
...
export default function DashboardPage() {
  ...
  // Outstanding = pending + approved + partial balances
  const outstanding = [...pending, ...approved, ...partial];

  // Sum helper — subtracts partialAmountPaid for partial claims
  const sum = (arr: Claim[]) =>
    arr.reduce((s, c) => {
      if (c.status === "partial" && c.partialAmountPaid) {
        return s + (c.amount - c.partialAmountPaid);
      }
      return s + c.amount;
    }, 0);
  ...
  // Per medical aid breakdown — balance aware
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-gray-900 text-white rounded-2xl px-7 py-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-0.5">Pending + Approved + Partial balances — money owed right now</p>
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">Active Claims — Money Being Tracked</p>
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <StatCard
              ...
              accent="bg-amber-400" icon="🕐" hint="263 has not yet approved or rejected"
            />
            <StatCard
            ...
            <StatCard
              ...
              accent="bg-orange-400" icon="⚡" hint="Balance remaining after partial payment"
            />
            <StatCard
              label="Overdue 60+ Days" sublabel="Approved or pending over 60 days ago"
              amount={sum(bucket60)} count={bucket60.length} countLabel="claims"
              accent="bg-red-500" icon="⚠️" hint="Chase these today — call the medical aid"
            />
          </div>
        </div>
        ...
        {/* Row 3: Resolved */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">Resolved This Month</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <ResolvedCard
              ...
              hint={rejected.length > 0 ? "Action needed — open claims to resubmit" : "All clear"}
            />
          </div>
        </div>
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {dueNow.slice(0, 3).map((c) => (
                  <div key={c._id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-orange-100">
                    <div>
                      <p className="text-sm font-medium text-gray-800">{c.patientName}</p>
                ...
                {dueNow.length > 3 && (
                  <p className="text-xs text-orange-500 text-center pt-1">+{dueNow.length - 3} more in the claims table</p>
                )}
              </div>
            </div>
          );
